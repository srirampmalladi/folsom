...
public class MemcacheClientBuilder<V> {
  ...
  /**
   * Lazily instantiated singleton default dnsResolver.
   */
  ...
  private RawMemcacheClient createSRVClient(final boolean binary) {
    DnsSrvResolver resolver = dnsResolver;
    if (resolver == null) {
      resolver = DefaultDnsResolver.INSTANCE;
    }

    return new SrvKetamaClient(srvRecord, resolver, DefaultScheduledExecutor.INSTANCE,
