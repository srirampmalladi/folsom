...
public abstract class AbstractRawMemcacheClient implements RawMemcacheClient {
  ...
  public void registerForConnectionChanges(ConnectionChangeListener listener) {
    eventBus.register(new Wrapper(listener));
  }
  ...
  private class Wrapper {
    private final ConnectionChangeListener listener;

    public Wrapper(ConnectionChangeListener listener) {
      this.listener = listener;
    }

    @Subscribe
    public void onEvent(RawMemcacheClient client) {
      listener.connectionChanged(client);
    }
  }
}
